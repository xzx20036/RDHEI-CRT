function [imge_extract,secret_img] = Extract(imge_embedded)
em_img=imge_embedded;
[input_rows, input_cols] = size(em_img);
img=em_img;
num_blocks_3x3_rows = input_rows / 3;  
num_blocks_3x3_cols = input_cols / 3;  
...
secret_img=dec_bin_text_recover(data,b);
new_blocks = im2col(img, [3, 3], 'distinct');
n = new_blocks(1, :);  % 还原原2×2块左上角
